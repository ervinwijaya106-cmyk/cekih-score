...
function calculateRanking(players) {
  ...
  indices.sort((a, b) => {
    const sa = players[a].score, sb = players[b].score;
    if (sb !== sa) return sb - sa;
    return a - b;
  });
  return indices; // array: ranking[0] = setupIdx of rank 1 player
}
...
function handleConfirmBurn() {
  ...
  // Play attack animation for each burn
  burnActionsForAudio.forEach((ba, idx) => {
    setTimeout(() => {
      playAttackAnimation(ba.attackerIdx, ba.victimIdx);
    }, idx * 800);
  });

  // Process burn after short delay (animations)
  setTimeout(() => {
  ...
  }, Math.max(1200, burnActionsForAudio.length * 800));
}
